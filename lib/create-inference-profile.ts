...
export class BedrockInferenceProfile extends Construct {
  ...
  constructor(scope: Construct, id: string, lambdaFunctions: lambda.Function[] = []) {
    super(scope, id);
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if request_type == 'Create' or request_type == 'Update':
            ...
            try:
                ...
                # Update Lambda functions with the inference profile ARN if specified
                lambda_functions = event.get('ResourceProperties', {}).get('lambdaFunctions', [])
                if lambda_functions:
                    print(f"Updating {len(lambda_functions)} Lambda functions with inference profile ARN")
                    ...
                    for function_name in lambda_functions:
                        try:
                            ...
                            env_vars['CLAUDE_INFERENCE_PROFILE_ARN'] = inference_profile_arn
                            ...
                            print(f"Error updating Lambda function {function_name}: {str(e)}")
                ...
                # Still try to update Lambda functions with the default ARN
                lambda_functions = event.get('ResourceProperties', {}).get('lambdaFunctions', [])
                if lambda_functions:
                    print(f"Updating {len(lambda_functions)} Lambda functions with default inference profile ARN")
                    lambda_client = boto3.client('lambda')
                    ...
                    for function_name in lambda_functions:
                        try:
                            ...
                            # Add or update the CLAUDE_INFERENCE_PROFILE_ARN variable
                            env_vars['CLAUDE_INFERENCE_PROFILE_ARN'] = default_inference_profile_arn
    ...
    // Add permissions for Lambda if functions are provided
    ...
    const inferenceProfileResource = new cdk.CustomResource(this, 'InferenceProfileResource', {
      ...
      properties: {
        ...
        // Add Lambda function names if provided
        lambdaFunctions: lambdaFunctions.map(fn => fn.functionName),
      },
    });
